...
```
```{r test filters}
counts_filtered <- filter_counts(all_counts, 0.5, 60)

counts_filtered

```
```{r scatter1}
scatter1 <- scatter_plot1(prep_scatter_data(all_counts), 0.5, 60)

scatter1

```
```{r scatter2}
scatter2 <- scatter_plot2(prep_scatter_data(all_counts), 0.5, 60)
